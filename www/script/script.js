$(document).ready(function(){
    ...
    // #direct_go를 누르면
        //아까 알아뒀던 눌린a의 href속성 값이 공백이 아니라면
            //아까 알아뒀던 눌린 a의 href속성 값으로 이동
    
    var clink;
    var cname;
    
    $("#direct_cont>a").click(function(event){
        event.preventDefault();
        clink = $(this).attr("href");
        cname = $(this).text();
        $("#direct_title>span").text(cname);
    });
    ...
    //tap 버튼에 따라 tabcont 보여주기
    //.tabbtn을 눌렀을때 
        //방금눌린그것이 몇번재 인가알아내서 th 라고 부르자.
        //모든 .tabcont를 모조리 숨겨줌
        //이번에 보여줘야할 .tabcont(탭콘트중에 th번째)를 보여줌.
    
    // list-db 안에 있는 li 안쪽에 넘버링하기
    // 각각의 list-db 에게 다음과 같이 하겠다.
        //li가 몇개가 있는지 개수를 파악.
        //그 개수만큼 반복
    ...
    // 리스트로 만든 표 안쪽에 셀들 높이 가장 큰놈으로 통일하기
    // .listtable>li 안에 있는 span들이 몇개인가 개수를 알아냄. => num
    // 그 개수만큼 반복
    ...
    function setheight(){
        $(".listtable>li").each(function(){
            if($(this).is(":visible")){
                ...
                for(i=0; i<num; i++)
                {
                    newdata = $(this).children("span").eq(i).height();
                    if(newdata >= olddata){
                        olddata = newdata;
                    }
                }
                $(this).children("span").height(olddata);
            }
        });   
    }
    ...
    // section table들에게 각각 다음과 같이 이야기 하겠다.
        // 그 녀석의 바로 윗 부분 에 "<div class='tablebox'>" 추가
...
//    
    // 문서에서 select태그를 찾아서 각각 다음과 같이 이야기 하겠다.
        // 그것의 속성중에 "name"이라는 속성의 값을 알아낸다. => bajiname
        // 그것의 다음에 "<div class='baji' data='+"bajiname"+'></div>"을 만든다.
        // 그것의 바로 다음에 있는 .baji를 선택해서 그 안에 추가한다.
        //      "<div class='baji-mark></div> <div class='baji-opt'></div>"
        // 그것의 안쪽에 있는 option들의 개수를 알아낸다. => selle
        // sellen개 만큼 반복   
            // i번째 option의 내용을 알아낸다 => opttxt
            // .baji-opt의 안쪽에 "<span>+opttxt+</span>"을 추가하는 것
        //그 .baji-opt>span 중에 첫번째 span의 내용을 갈무리해서
        //그 .baji-mark의 내용으로 넣어준다.
        //원본 그것(select)을 숨겨 준다.
        //챔피언 = 0
        // 첫번째 도전자 vs 챔피언 (숫자가 더 큰 사람이 이김)
        // 두번째 도전자 vs 챔피언 (첫번재 도전자)
        // 세번째 도전자 vs 챔피언 (1?2?)
        //마지막에 최종 챔피언(숫자가 가장 큰 사람)
    
    $("select").each(function(){
        var bajiname = $(this).attr("name");
        $(this).after("<div class='baji' data='"+bajiname+"'></div>");
        $(this).next(".baji").append("<div class='baji-mark'></div><div class='baji-opt'></div>");
    ...
    // .baji를 누르면
        // 그것 안쪽에 들어있는 .baji-opt을 보였다가 / 안보였다가
    $(".baji").click(function(){
    ...
    // .baji-opt>span을 눌렀을대
        // 방금누른 그것 안에 있는 내용을 갈무리 => spantxt
        // 방금누른 그것의 부모의 형에게 그 내용을 써준다.
        // 방금 누른 그것이 span중에서 몇번째 였던가? => th
        // 방금 누른 그것의 부모의 부모의 형(select)의 
        // 자식중에 option중에서 th번째를 강제로 선택한다.
